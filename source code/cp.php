...
elseif(isset($_GET['toggle_sms_reminder']))
{
	echo toggle_sms_reminder();
}



elseif(isset($_GET['toggle_email_verification']))
{
	echo toggle_email_verification();
}

elseif(isset($_GET['toggle_email_reminder']))
{
	echo toggle_email_reminder();
}
elseif(isset($_GET['get_sms_reminders']))
{
	echo get_sms_reminders();
}
elseif(isset($_GET['get_email_reminders']))
{
	echo get_email_reminders();
}

elseif(isset($_GET['who_can_register']))
{
   $who_can_id = mysql_real_escape_string(trim($_POST['who_can_id']));
	$who_can_value = mysql_real_escape_string($_POST['who_can_value']);
	echo update_who_can_register($who_can_id, $who_can_value);
}

elseif(isset($_GET['change_url']))
{
   $new_url = $_POST['text_input'];
	echo change_url($new_url);
}
...
elseif(isset($_GET['update_sms_details']))
{
	$user_name = mysql_real_escape_string(trim($_POST['user_name']));
	
	echo update_sms_details($user_name);
}

elseif(isset($_GET['update_email_details']))
{
	$user_name = mysql_real_escape_string(trim($_POST['user_name']));
	
	echo update_email_details($user_name);
}

elseif(isset($_GET['update_welcome_email_msg']))
{
	$user_name = mysql_real_escape_string(trim($_POST['user_name']));
	
	echo update_welcome_email_msg($user_name);
}

elseif(isset($_GET['registration_settings']))
{
	$user_name = mysql_real_escape_string(trim($_POST['value_id']));
	
	echo registration_settings($user_name);
}


elseif(isset($_GET['registration_enabling']))
{
	$user_name = mysql_real_escape_string(trim($_POST['enable_id']));
	
	echo registration_enabling($user_name);
}
